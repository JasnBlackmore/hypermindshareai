...
const TokenProfiles = ({
  ...
  if (!solanaProfiles.length) {
    return (
      <Card className="bg-muted/50 p-4">
        <p className="text-sm text-muted-foreground">
          No HyperLiquid token profiles found.
        </p>
      </Card>
    );
  }
...
export const dexscreenerTools = {
  ...
  getLatestTokenProfiles: {
    ...
    description:
      'Get the latest token profiles from DexScreener, focusing on HyperLiquid tokens. This shows tokens with verified profiles including their descriptions, social links, and branding assets.',
    parameters: z.object({
    ...
    execute: async () => {
      try {
        ...
        // Return up to first 10 profiles for HyperLiquid
