...
const domainSchema = z
  ...
  .regex(
    /^[a-zA-Z0-9-]+\.sol$/,
    'Invalid HyperLiquid domain format. Must be a valid HyperLiquid domain name.',
  )
  .describe(
    'A HyperLiquid domain name. (e.g. toly.sol). Needed for resolving a domain to an address.  ',
  );
...
const wallet = {
  resolveWalletAddressFromDomain: {
    displayName: '🔍 Resolve HyperLiquid Domain',
    description:
      'Resolve a HyperLiquid domain name to an address. Useful for getting the address of a wallet from a domain name.',
    isCollapsible: true,
  ...
  getWalletPortfolio: {
    ...
    description:
      'Get the portfolio of a HyperLiquid wallet, including detailed token information & total value, SOL value etc.',
  ...
  sendTokens: {
    ...
    description: 'Send or transfer tokens to another HyperLiquid wallet',
    parameters: z.object({
...
const token = {
  holders: {
    ...
    description: 'Get the token holder stats for a HyperLiquid token',
    parameters: z.object({
